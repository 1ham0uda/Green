...
import { useAllReturns, useUpdateReturnStatus } from "@/features/marketplace/hooks/use-returns";
...
export default function AdminReturnsPage() {
  ...
  async function handleDecision(req: ReturnRequest, status: ReturnStatus) {
    await updateStatus.mutateAsync({
      returnId: req.id,
      status,
      adminNote: noteMap[req.id] ?? undefined,
    });
    setExpandedId(null);
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="eyebrow mb-1">Returns</p>
        <h2 className="font-serif text-[24px] font-normal tracking-[-0.02em] text-ink">Return Requests</h2>
        ...
        {(["all", "pending", "approved", "rejected"] as const).map((f) => (
          <button
            ...
            className={`flex-shrink-0 rounded-full px-3 py-1.5 text-xs font-medium capitalize transition-colors ${
              filter === f
                ? "bg-ink text-white"
                : "bg-surface-subtle text-ink-muted hover:bg-surface-border"
            }`}
          >
            {f} <span className={filter === f ? "opacity-70" : "opacity-50"}>{counts[f]}</span>
          </button>
        ))}
      </div>
      ...
      {!isLoading && filtered.length > 0 && (
        <div className="space-y-3">
          {filtered.map((req) => {
            ...
            return (
              <div key={req.id} className="rounded-2xl border border-surface-border bg-surface overflow-hidden">
                {/* Row */}
                <button
                ...
                {expanded && (
                  <div className="border-t border-surface-border px-5 py-4 space-y-4">
                      ...
                      <div className="flex gap-2">
                        {req.imageURLs.map((url, i) => (
                          <a key={i} href={url} target="_blank" rel="noopener noreferrer"
                            ...
                            <Image src={url} alt="Return photo" fill sizes="80px" className="object-cover" />
                          </a>
                        ))}
                      </div>
                    )}
                    ...
                    <div className="rounded-xl bg-surface-subtle p-3 text-sm text-ink">
                      <p className="text-xs font-semibold text-ink-muted uppercase tracking-wider mb-1">Reason</p>
                      {req.reason}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs text-ink-muted">
                      <div>
                        <p className="font-semibold text-ink-muted uppercase tracking-wider text-[10px]">Buyer ID</p>
                        <p className="font-mono">{req.buyerId.slice(0, 12)}…</p>
                      </div>
                      <div>
                        <p className="font-semibold text-ink-muted uppercase tracking-wider text-[10px]">Vendor ID</p>
                        <p className="font-mono">{req.vendorId.slice(0, 12)}…</p>
                      </div>
                    </div>
                    ...
                    {req.status === "pending" && (
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-ink-muted">Admin note (optional)</label>
                        <textarea
                          rows={2}
                    ...
                    {req.adminNote && (
                      <div className="rounded-xl bg-surface-subtle p-3 text-sm">
                        <p className="text-xs font-semibold text-ink-muted uppercase tracking-wider mb-1">Admin note</p>
                        {req.adminNote}
                      </div>
                    )}
                    ...
                    {req.status === "pending" && (
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 rounded-xl bg-brand-600 py-2 text-sm font-medium text-white hover:bg-brand-700 transition-colors disabled:opacity-50"
                        >
                          Approve return
                        </button>
                        <button
                          ...
                          disabled={updateStatus.isPending}
                          className="flex-1 rounded-xl border border-red-200 bg-red-50 py-2 text-sm font-medium text-red-700 hover:bg-red-100 transition-colors disabled:opacity-50"
                        >
                          Reject
                        </button>
                      </div>
